...
export default function LikeButton({ id }: LikeButtonProps) {
    ...
    return (
        <button
            onClick={handleLike}
            className={`group flex items-center justify-center gap-1.5 px-2 py-2 rounded-full transition-all duration-300 fixed bottom-8 right-8 z-50 ${liked
                ? 'text-rose-600'
        ...
        >
            <Heart
                size={24}
                className={`transition-colors duration-300 drop-shadow-sm ${liked ? 'fill-rose-600' : 'group-hover:fill-rose-500/10'
                    }`}
                strokeWidth={2}
            />
